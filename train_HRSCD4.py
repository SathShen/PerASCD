...
from models.HRSCD import HRSCD4, HRSCD3
NET_NAME = 'HRSCD3'
DATA_NAME = 'SECOND'
###############################################
# Training options
...
args = {
    ...
    'predict_step': 5,
    'pseudo_thred': 0.6,
    'log_dir': f'/data2/sht/Outputs/SCD/runs/{DATA_NAME}/{NET_NAME}/{int(time.time())}',
    'load_path': None
}
###############################################
...
def main():
    ...
    if NET_NAME == 'HRSCD4':
        net = HRSCD4(3, num_classes=RS.num_classes).cuda()
    elif NET_NAME == 'HRSCD3':
        net = HRSCD3(3, num_classes=RS.num_classes).cuda()
    else:
        raise ValueError('Invalid network name: %s' % NET_NAME)
...
def train(train_loader, net, optimizer, val_loader, curr_epoch=0, bestFscdV=0.0):
    ...
    while True:
        ...
        train_lcd = AverageMeter()
        train_lcm = AverageMeter()
        if NET_NAME == 'HRSCD4' or NET_NAME == 'HRSCD3':
            criterion_Lcd = BCEWithLogitsLoss().cuda()
        criterion_Lcm = CrossEntropyLoss2d(ignore_index=0).cuda()
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            with autocast(device_type='cuda', dtype=torch.float16):
                ...
                if NET_NAME == 'HRSCD4':
                    loss_cm = criterion_Lcm(outputs_A, labels_A) + criterion_Lcm(outputs_B, labels_B)
                    loss_cd = criterion_Lcd(out_change, labels_cd)
                    loss = loss_cd + 0.05 * loss_cm
                elif NET_NAME == 'HRSCD3':
                    loss_cm = criterion_Lcm(outputs_A, labels_A) + criterion_Lcm(outputs_B, labels_B)
                    loss_cd = criterion_Lcd(out_change, labels_cd)
                    loss = loss_cd + 0.05 * loss_cm
            ...
            #     labels_unchange = torch.logical_not(labels_cd).squeeze()
            ...
            train_lcd.update(loss_cd.cpu().detach().numpy())
            train_lcm.update(loss_cm.cpu().detach().numpy())

            writer.add_scalar('train_lcd', train_lcd.val, running_iter)
