...
NOTE = 'Lr1e1min0W01C15D03LsscTau001Train01'
###############################################
# Training options
###############################################
args = {
    ...
    'predict_step': 5,
    'pseudo_thred': 0.6,
    'log_dir': f'/data2/sht/Outputs/SCD/runs/{DATA_NAME}/{NET_NAME}/{NOTE}_{int(time.time())}',
    'load_path': None
}
###############################################
...
def main():
    ...
    train_set = RS.DataAUG('train', random_flip=True, random_swap=False, path=DATA_PATH)
    ...
    val_set = RS.DataAUG('test', path=DATA_PATH)
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=args['lr'], weight_decay=1e-5, momentum=0.9, nesterov=True)
    curr_epoch = 0
    bestFscdV = 0.0
...
def train(train_loader, net, criterion, optimizer, val_loader, curr_epoch=0, bestFscdV=0.0):
    ...
    criterion_sc = SoftSemanticConsistency(reduction='mean', tau=0.01).cuda()
    ...
    while True:
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            with autocast(device_type='cuda', dtype=torch.float16):
                ...
                nn.utils.clip_grad_norm_(net.parameters(), 1.5)
        ...
        print('Epoch: %d Total time: %.1fs Best rec: Train acc %.2f, Val Fscd %.2f acc %.2f loss %.4f' 
              %(curr_epoch, time.time()-begin_time, bestaccT*100, bestFscdV*100, bestaccV*100, bestloss))
        curr_epoch += 1
...
def adjust_lr(optimizer, iter_ratio, init_lr=args['lr'], warmup_ratio=0.1, min_lr=0.):
    # warmup
