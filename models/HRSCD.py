...
class HRSCD4(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, wsl=False):
        super(HRSCD4, self).__init__()

        self.input_nbr = in_channels
        self.label_nbr = num_classes
...
class HRSCD3(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, wsl=False):
        ...
        # Terrain Classification (Land Cover Mapping)
        self.TCEncoder = Encoder(self.input_nbr)
        self.TCDecoder = Decoder(self.label_nbr, self.TCEncoder.depths)

        # Change Detection (binary CD)
        self.CDEncoder = Encoder(2 * self.input_nbr)
        if wsl:
            self.CDDecoder = Decoder(2, self.CDEncoder.depths)  # weakly supervised
        else:
            self.CDDecoder = Decoder(1, self.CDEncoder.depths)  # binary CD

    def forward(self, x1, x2):
        # Land Cover Mapping for both images
        outputs_1, sizes_1 = self.TCEncoder(x1)
        tc1 = self.TCDecoder(outputs_1, sizes_1)
...
class HRSCD2(nn.Module):
    def __init__(self, in_channels=3, num_classes=7):
        ...
        输入: 两张影像拼接 (2 * in_channels)
        输出: 所有可能的变化类别 (num_classes^2)
        """
        super(HRSCD2, self).__init__()

        self.input_nbr = 2 * in_channels
        self.label_nbr = num_classes * num_classes  # 所有变化组合类别
        self.num_classes = num_classes

        # 语义变化检测网络
        self.Encoder = Encoder(self.input_nbr)
        self.Decoder = Decoder(self.label_nbr, self.Encoder.depths)

    def post_process(self, scm):
        pred = scm.argmax(dim=1)

        # 还原成两个时刻的类别
        tc1 = pred // self.num_classes
        tc2 = pred % self.num_classes

        # 变化检测图
        cm = (tc1 != tc2).long()

        # 增加通道维度
        tc1_onehot = torch.zeros(tc1.size(0), self.num_classes, tc1.size(1), tc1.size(2)).to(tc1.device)
        tc1_onehot.scatter_(1, tc1.unsqueeze(1), 1)
        tc2_onehot = torch.zeros(tc2.size(0), self.num_classes, tc2.size(1), tc2.size(2)).to(tc2.device)
        tc2_onehot.scatter_(1, tc2.unsqueeze(1), 1)
        cm = cm.unsqueeze(1)

        return cm, tc1_onehot, tc2_onehot
    
    def double_label_2_single_label(self, label1, label2):
        # 将两个时刻的标签合并成一个标签
        # label1, label2: (B, H, W)
        label = label1 * self.num_classes + label2
        return label
    ...
    def forward(self, x1, x2):
        ...
        # 编码-解码
        outputs, sizes = self.Encoder(x)
        scm = self.Decoder(outputs, sizes)  # semantic change map
