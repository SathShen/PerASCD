...
class ChangeAwareGatingModule(nn.Module):
    ...
    def forward(self, x):
        ...
        change_weight = local_weight * (1 + global_weight)
...
class PerASCD(nn.Module):
    def __init__(self, 
        ...
        self.decoder = CascadeGatedDecoder([embed_dim] * 4, dim, 3, droppath)
        ...
        self.classifierA = nn.Conv2d(dim, num_classes, kernel_size=1)
        self.classifierB = nn.Conv2d(dim, num_classes, kernel_size=1)
        self.classifierCD = nn.Sequential(nn.Conv2d(dim, dim // 2, kernel_size=1), nn.BatchNorm2d(dim // 2), nn.ReLU(), nn.Conv2d(dim // 2, 1, kernel_size=1))
            
        initialize_weights(self.decoder, self.classifierA, self.classifierB, self.classifierCD)
    ...
    def forward(self, x1, x2):
        ...
        x1, x2, xc = self.decoder(outputs_1, outputs_2)
