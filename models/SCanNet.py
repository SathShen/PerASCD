...
class SCanNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, input_size=512):
        ...
        self.resCD = self._make_layer(ResBlock, 256, 128, 6, stride=1, drop_path=0.3)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, drop_path=0.):
        downsample = None
        if stride != 1 or inplanes != planes:
            downsample = nn.Sequential(
                conv1x1(inplanes, planes, stride),
        ...
        layers = []
        layers.append(block(inplanes, planes, stride, downsample, drop_path=drop_path))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
    ...
    def forward(self, x1, x2):
        ...
        x1, x1_low = self.base_forward(x1)  # b 128 64 64, b 64 128 128
        x2, x2_low = self.base_forward(x2)
        x1, x2, xc = self.CD_forward(x1, x2) # xc: b 128 64 64

        x1 = self.Dec1(x1, x1_low)  # b 128 128 128
        x2 = self.Dec2(x2, x2_low)       # b 128 128 128 
        xc_low = torch.cat([x1_low, x2_low], 1)  # b 128 128 128
        xc = self.DecCD(xc, xc_low)  # b 128 128 128
        ...
        return F.interpolate(change, x_size[2:], mode='bilinear', align_corners=False), \
               F.interpolate(out1, x_size[2:], mode='bilinear', align_corners=False), \
               F.interpolate(out2, x_size[2:], mode='bilinear', align_corners=False)