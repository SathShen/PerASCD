...
class Extractor(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1,
                 with_cffn=True, cffn_ratio=0.25, drop=0., drop_path=0.,
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), with_cp=False):
        super().__init__()
        self.query_norm = norm_layer
        self.feat_norm = norm_layer
...
class SpatialPriorModule(nn.Module):
    def __init__(self, inchannel, inplanes=64, embed_dim=384, with_cp=False):
        super().__init__()
        self.with_cp = with_cp

        self.stem = nn.Sequential(*[
            nn.Conv2d(inchannel, inplanes, kernel_size=3, stride=2, padding=1, bias=False),
